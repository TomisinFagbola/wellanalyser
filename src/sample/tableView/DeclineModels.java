...
public class DeclineModels implements Initializable {
    ...
    private Label mode;
    @FXML
    private ChoiceBox wellType;
    @FXML
    private ChoiceBox declineType;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        wellType.getItems().add("Vertical");
        wellType.getItems().add("Horizontal");

        declineType.getItems().add("Exponential Decline");
        declineType.getItems().add("Harmonic");
        declineType.getItems().add("Hyperbolic");
        modeSelected = ED_MODE;
        mode.setText(getActiveModeText());
        vertical = true;
        wellType.setOnAction((event) -> {
            int selectedIndex = declineType.getSelectionModel().getSelectedIndex();
            Object selectedItem = declineType.getSelectionModel().getSelectedItem();
            if (selectedIndex == 0) {
                vertical = true;
            } else {
                vertical = false;
    ...
    private void displayResult(Double qo, Double qf, Double npd, Double a, Double tf, Double b) throws BackingStoreException {
        ...
        ((Stage) resultLabel.getScene().getWindow()).close();
    ...
    private void generateDailyProduction(DeclineModel model) {
        ...
        Well well = new Well();
        well.setDeclineModel(model);
        well.setWellId(UUID.randomUUID().toString());
        well.setNameOfWell("Well " + new Random().nextInt(1000));
        well.setYearlyProductions(result);
        well.setVertical(vertical);
        saveWell(well);
    ...
    private void generateDailyProductionHyperbolic(DeclineModel model) {
        ...
        try {
            ...
            while (initialQ >= model.getQf()) {
                finalNpd = initialNpd - initialQ;
                initialQ = model.getQo() * Math.pow(1 + ((model.getB() * counter) / model.getA()), -1 / model.getB());
                finalQ = initialQ;
                initialNpd = finalNpd;
                yearlyProductionModels.add(new DeclineModel(finalQ, model.getQf(), finalNpd, model.getA(), model.getB(), 0.0, modeSelected));
                System.out.println(new Gson().toJson(new DeclineModel(finalQ, model.getQf(), finalNpd, model.getA(), model.getB(), model.getTf(), modeSelected)) +" ------>>>  "+ Math.pow(1 + ((model.getB() * counter) / model.getA()), -1 / model.getB()));
                counter++;
            }
            counter = 2;
            Double initNpd, finNpd;
            ...
            for (int i = 1; i < Math.ceil(tfMonth) + 1; i++) {
                if (i == Math.ceil(tfMonth)) {
                    ...
                    DecimalFormat df = new DecimalFormat("#.#");
                    result.add(new YearlyProduction(df.format(tfMonth), (initNpd - finNpd)));
                    System.out.println(df.format(tfMonth) + "  Production Per Year: " + (initNpd - finNpd) +"   === "+ initNpd+" - "+ finNpd);
                } else {
                    finNpd = yearlyProductionModels.get(yearAdd).getNpd();
                    result.add(new YearlyProduction(String.valueOf(i), (initNpd - finNpd)));
                    //System.out.println(i + "  Production Per Year: " + (initNpd - finNpd));
                    System.out.println(i + "  Production Per Year: " + (initNpd - finNpd) +"   === "+ initNpd+" - "+ finNpd);
    ...
    private void generateDailyProductionHarmonic(DeclineModel model) {
        ...
        try {
            ...
            while (initialQ >= model.getQf()) {
                ...
                initialQ = model.getQo() * Math.pow(1 + (counter / model.getA()), -1);
                finalQ = initialQ;
                initialNpd = finalNpd;
                yearlyProductionModels.add(new DeclineModel(finalQ, model.getQf(), finalNpd, model.getA(), model.getB(), 0.0, modeSelected));
                System.out.println(new Gson().toJson(new DeclineModel(finalQ, model.getQf(), finalNpd, model.getA(), model.getB(), model.getTf(), modeSelected)));
    ...
    private void analyseHarmonic(Double qo, Double qf, Double npd, Double a, Double tf, Double b) throws BackingStoreException {
        final double v = ((qo / qf) - 1) / Math.log(qo / qf);
        if (qo != null && qf != null && npd != null) {
            tf = (npd / qo) * v;
            a = tf / ((qo / qf) - 1);
            displayResult(qo, qf, npd, a, tf, b);
    ...
    private void saveWell(Well well) {
        ...
        for (int i = 0; i < companyModels.size(); i++) {
            if (companyModels.get(i).getPrimary()) {
                ArrayList<Field> fields = companyModels.get(i).getFields();
                for (int j = 0; j < fields.size(); j++) {
                    if (fields.get(j).getPrimary()) {
                        Field field = fields.get(j);
                        ArrayList<Well> wells;
                        if (null == companyModels.get(i).getFields().get(j).getWells()) {
                            wells = new ArrayList<>();
                        } else {
                            wells = companyModels.get(i).getFields().get(j).getWells();
                        }
                        wells.add(well);
